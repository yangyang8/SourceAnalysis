 ...
 * 
 * Wrapper级的Servlet容器表示的是一个org.apache.catalina.Wrapper接口的实例，表示的是一个单独的Servlet的定义
 *Wrapper接口继承于Container接口，又接口了一些额外的方法，Wrapper的接口的实例类要负责管理其基础的Servlet类的
 *Servlet的类的生命周期，也就是调用Servlet的init(),service(),destory()等方法，
 *又因为我们Wrapper已经是最低等级的Servlet的Container的容器，所以不能再向其增加子容器
 *如果这个Wrapper接口的addChild()方法被调用，则会抛出ILLegalArrgumantException异常
 *
 *这个接口最重要的二个方法为load()方法和allocate();
 *allocate()方法会分配一个已经初始化的Servlet实例，而且allocate方法不要考滤该Servlet类有没有实例
 *SingleThreadModel接口
 *
 *我们的load()方法主要是进行加载并初始化我们的Servlet类
 */
