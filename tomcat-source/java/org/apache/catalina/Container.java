 ...
 *
 *
 *我们容器和连接器的关联是通过我们的连接器的setContainer的方法来实现的，这样子我们的连接器才可以调用
 *Servlet的invoke的方法
 * @author Craig R. McClanahan
 * @author Remy Maucherat
 * 一个Container容器对象可以接收到客户端的请求，并可以执行客户端的请求，同时在请求的基础上返回
 * 相应响应给我们的客户端
 * Container容器当中包含了支持的一些组件，如载入器，记录器，管理器，领域，资源，同时
 * 我们的Container容器提供了getter/setter方法进行和容器进行相关联
 * 
 * 同时我们在部暑这个tomcat的时候可以通过修改这个tomcat的配置文件(server.xml)来决定进行使用那个容器
 * 原理是通过我们我们容器当中的管道(pipeline)和value的集合来实现的
 */
