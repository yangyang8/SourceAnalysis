 ...
 * 
 * 这个catalina类可以关闭/开启我们tomcat服务器
 * 同时Tomcat的BootStrap类也是调用了我们的Catalina的类的方法进行关闭/开启我们tomcat服务器
 * 最为重要的就他也在这个类当中使用了我们ServerSocket
 */
public class Catalina {
    ...
    public Server getServer() {
    	//StandardServer  ==>这个server为StandardServer类
        return server;
    }
    ...
    /**
     * 这个方法在启动的时候被调用，然后修改我们我们await值，为true
     * @param b
     */
     ...
     * 读取我们tomcat的配置文件server.xml
     */
    protected File configFile() {
    	//file 就是conf/server.xml
        File file = new File(configFile);
        if (!file.isAbsolute()) {
            file = new File(System.getProperty(Globals.CATALINA_BASE_PROP), configFile);
        }
        //F:\JavaWeb_workspace\tomcat-source\conf\server.xml
        return (file);
    ...
    //初始化目录
    /**
     * 也就是判断一下我们的catalina.home和"catalina.base"是否为空，然后把这二个目录的值设置进行系统当中
     * 最后再判断一下临时目录
     */
    protected void initDirs() {
    	//catalinaHome==F:\JavaWeb_workspace\tomcat-source项目工程目录
        ...
        if (catalinaHome != null) {
        	//创建一个F:\JavaWeb_workspace\tomcat-source工程项目的文件
            ...
            //给 CATALINA_HOME_PROP也就是"catalina.home"进行赋值为F:\JavaWeb_workspace\tomcat-source
            System.setProperty(Globals.CATALINA_HOME_PROP, catalinaHome);
        }
        //进行判断一下我们的CATALINA_BASE_PROP也就是CATALINA_BASE_PROP是否为空
        if (System.getProperty(Globals.CATALINA_BASE_PROP) == null) {
            System.setProperty(Globals.CATALINA_BASE_PROP,
                               catalinaHome);
        } else {
        	//catalinaBase的值为F:\JavaWeb_workspace\tomcat-source
            ...
            //给 CATALINA_BASE_PROP也就是""catalina.base""进行赋值为F:\JavaWeb_workspace\tomcat-source
            System.setProperty(Globals.CATALINA_BASE_PROP, catalinaBase);
        }
        //创建一个目录值为C:\Users\ADMINI~1\AppData\Local\Temp\
    ...
    protected void initNaming() {
        ...
        if (!useNaming) {
            ...
            //设置java.naming.factory.url.pkgs=org.apache.naming
