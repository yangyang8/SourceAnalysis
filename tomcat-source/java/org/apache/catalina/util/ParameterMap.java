 ...
 * @param <V> Value type
 * 
 * 有个locked标志，用来记录我们这个参数ParameterMap是否可以进行修改的操作
 * 为false则表不能修改，true则表示可以进行修改
 * 
 * 因为我们的Servlet开发人员是不能进行修改我们HTTP请求的参数，所以这个数据结果在增删改时要用到
 * locked标志来判断一下我们ParameterMap数据是否可以进行修改
 * 
 */
