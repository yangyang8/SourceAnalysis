 ...
 * 
 * 这个类最重要的二个方法为start()和stop(),组件一定要实现这二个方法，供父类组件进行调用
 * 以实现对其的启动和停止
 * 
 * addLifecycleListener(LifecycleListener)
 * removeLifecycleListener(LifecycleListener)
 * findLifecycleListeners()
 * 这上面的三个方法都是和我们的事件监听器相关人，我们的组件当中可以注册多个事件监听器对发生的
 * 该组件上的一些事进行监听，当一些事件发生时，相应的事件监听器就会收到相应通知
 */
